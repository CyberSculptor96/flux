...
# 提取 n=50 的各项指标
n50_clip = [d['n50'][0] for d in sorted_data]
n50_ir   = [d['n50'][1] for d in sorted_data]
n50_aes  = [d['n50'][2] for d in sorted_data]

# 3. 设置绘图风格
plt.style.use('bmh') # 使用一种比较干净的科研风格
fig, axes = plt.subplots(1, 3, figsize=(18, 5)) # 1行3列
fig.suptitle('Ablation Study Analysis of Mu (n=10, n=20 and n=50)', fontsize=16, y=1.05)

# 定义画图辅助函数
def plot_metric(ax, x, y1, y2, y3, title, ylabel):
    ax.plot(x, y1, marker='o', label='n=10', linewidth=2, linestyle='-')
    ax.plot(x, y2, marker='^', label='n=20', linewidth=2, linestyle='--')
    ax.plot(x, y3, marker='s', label='n=50', linewidth=2, linestyle=':')
    ...
    for i, txt in enumerate(y3):
        ax.annotate(f"{txt:.4f}", (x[i], y3[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=8)

# 4. 绘制三个子图
# 子图 1: CLIP Score
plot_metric(axes[0], mu_vals, n10_clip, n20_clip, n50_clip, 'CLIP Score', 'Score')
# # 子图 2: Image Reward
# plot_metric(axes[1], mu_vals, n10_ir, n20_ir, 'Image Reward', 'Reward')

# 子图 3: Aesthetic Score
plot_metric(axes[1], mu_vals, n10_aes, n20_aes, n50_aes, 'Aesthetic Score', 'Score')

# 子图 2: Image Reward
plot_metric(axes[2], mu_vals, n10_ir, n20_ir, n50_ir, 'Image Reward', 'Reward')

plt.tight_layout()
plt.savefig("mu_ablation_all.png", dpi=300, bbox_inches='tight')
